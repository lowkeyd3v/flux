  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-neutral-800">Backend Connection</h2>
        <button
          onClick={refetch}
          className="text-xs px-2 py-1 rounded-md border border-neutral-300 hover:bg-neutral-50 transition"
        >
          Refresh
        </button>
      </div>
      ...
      {status === 'error' && (
        <p className="mt-3 text-xs text-red-600">
          {error?.message || 'Could not reach the backend API. Is it running?'}
        </p>
      )}
    </div>
  )
