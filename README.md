...
> **Status: Milestone 4 (Recommendation engine + weather) complete.** This README reflects only what is currently implemented. Sections describing future milestones (RAG, voice) are marked as planned and will be filled in as those features are built — see [Project Status](#project-status).
...
| Recommendation engine + weather | Done (Milestone 4) |
...
Nothing below this point describes a shipped feature unless explicitly stated. Service interfaces for the still-unbuilt AI/ML components (RAG, voice) exist as clean, typed abstractions (see `backend/app/services/`) but currently raise `NotImplementedError` — this is intentional, so the codebase never pretends to have functionality it doesn't yet have.
...
| `POST` | `/api/vendors/{vendor_id}/recommend` | Recommend how much to prepare for a given date — combines the demand prediction with the vendor's inventory/budget/price and (if `WEATHER_API_KEY` is set) auto-fetched weather |

Full interactive documentation (Swagger UI) is auto-generated by FastAPI at `/docs` once the backend is running. This table will grow as milestones add real endpoints (scheme Q&A).
...
Current test coverage: health check endpoint smoke tests (`backend/tests/test_health.py`).
...
See [Project Status](#project-status) for the milestone roadmap: demand forecasting, recommendation engine, government scheme RAG, dashboard integration, and — time permitting — Hindi/Hinglish voice support.
...
- As of Milestone 4, vendor profiles, sales history, demand predictions, and recommendations (with weather) work end-to-end, but the government scheme RAG and voice features are not implemented yet — only their interfaces.
- Auto-fetched weather (via OpenWeatherMap) only covers today plus the next 5 days (free-tier forecast horizon). Recommendations for dates further out either need manual `temperature_celsius`/`weather_condition` input or fall back to the demand model's neutral defaults.
