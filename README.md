...
| Vendor profile + sales data | Planned (Milestone 2) |
...
│   │   ├── models/       # SQLAlchemy ORM models (added in Milestone 2)
│   │   ├── schemas/      # Pydantic request/response schemas (Milestone 2+)
...
| `WEATHER_API_KEY` | Reserved for weather service (Milestone 4) | empty |
...
Full interactive documentation (Swagger UI) is auto-generated by FastAPI at `/docs` once the backend is running. This table will grow as milestones add real endpoints (vendor profiles, predictions, recommendations, scheme Q&A).
...
- As of Milestone 1, no ML model, RAG pipeline, or recommendation logic exists yet — only their interfaces. Do not expect predictions or scheme answers until later milestones land.
